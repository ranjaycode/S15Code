...
**C budget-aware**    starts on the rung the ladder's ``role_tiers`` declares and
                      ...
                      admitted, metered and possibly downgraded by the S15Code
                      hard controller.

Every strategy runs through :class:`~s15code.economics.BudgetedGateway`, so all
...
:mod:`s15code.evals`. The task set is a data file; swap it for your own:
...
from s15code.economics import (  # noqa: E402
    BudgetedGateway,
    BudgetRefused,
    EconomicsConfig,
    MeteredTransport,
...
from s15code.evals import EvalsConfig, RubricJudge, Verdict, load_tasks  # noqa: E402
from s15code.evals.judge import STATUS_JUDGE_FAILED  # noqa: E402
from s15code.evals.tasks import EvalTask, by_difficulty  # noqa: E402
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--config-dir", default=os.getenv("S15_CONFIG_DIR"))
...
def run(parsed: argparse.Namespace) -> Proof:
    ...
    if parsed.judge_provider:
        from s15code.evals import JudgeModel
