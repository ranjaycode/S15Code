...
:class:`~s15code.gateway.GatewayClient`. It has to: the thing under test is the
gateway's ``cache`` envelope and its ``semantic_cache`` opt-in, and the client
deliberately does not carry either — S15Code's budget path must never be able to
