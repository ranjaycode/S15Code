...
from s15code.evals.judge import STATUS_JUDGE_FAILED, STATUS_RESOLVED, STATUS_UNRESOLVED
...
def test_rubric_threshold_and_weights_come_from_the_config_file():
    ...
    literals = _string_literals(Path(__file__).resolve().parents[1] / "s15code" / "evals")
...
async def test_an_invented_task_file_with_invented_criteria_works_unchanged(tmp_path):
    ...
    invented.write_text(
        "# a domain no s15code module has heard of\n"
...
def test_the_shipped_task_file_is_data_and_spans_difficulty():
    ...
    library = Path(__file__).resolve().parents[1] / "s15code"
...
async def test_the_judges_own_meta_cost_is_metered_separately():
    from s15code.economics import EconomicsConfig
