...
class FakeGateway:
    ...
    async def chat(self, *, prompt, system, request=None):
        ...
        return {"text": "a grounded answer", "provider": "prov_1",
...
def test_a_ceiling_too_small_for_any_call_refuses_rather_than_overspending(budgeted_client):
    ...
    # The refusal is a visible graph outcome, not a silent truncation.
    assert body["status"] == "failed"
    assert any(event["kind"] == "task_failed" for event in body["events"])
