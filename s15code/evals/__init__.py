...
* the **rubric** is fixed, generic and config-owned (:mod:`s15code.evals.config`).
...
* the **task** is data (:mod:`s15code.evals.tasks`). A task file may carry a short
...
:mod:`s15code.evals.judge` runs that rubric through the same gateway contract the
