...
nothing in ``s15code`` names a provider, a model, a price or a threshold. The
directory is resolved from ``S15_CONFIG_DIR`` when set, else ``config/`` beside
...
def config_dir(explicit: str | Path | None = None) -> Path:
    """Where the config lives: the argument, then ``S15_CONFIG_DIR``, then default."""
    if explicit is not None:
        return Path(explicit).expanduser().resolve()
    from_env = os.getenv("S15_CONFIG_DIR")
    if from_env:
