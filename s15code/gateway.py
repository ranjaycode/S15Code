"""The only S15Code -> gateway seam: ordinary authenticated HTTP.

S15Code holds no provider credential. It asks the gateway for a completion; the
...
class GatewayClient:
    ...
    def _payload(self, prompt: str, system: str, request: dict[str, Any] | None) -> dict[str, Any]:
        payload: dict[str, Any] = {
            ...
            # the independently metered gemini_1..N key pool; S15Code never sees
            ...
            "provider": os.getenv("S15_GATEWAY_PROVIDER", "gemini"),
        }
    ...
    async def chat(
        ...
        This is the ``ChatTransport`` protocol in :mod:`s15code.economics`: on a
        ...
        response = await self._client.post(
            f"{self.base_url}/v1/chat", json=self._payload(prompt, system, request)
        )
